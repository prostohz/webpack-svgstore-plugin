...
class WebpackSvgStore {
  ...
  createTaskContext(expr, parser) {
    ...
    if (data.prefix) {
      this.options.prefix = (this.prefix) ? this.prefix + data.prefix + '_': data.prefix;
    } else {
    ...
    let replacement = expr.id.name + ' = { filename: ' + "__webpack_require__.p +" + '"' + data.fileName + '", prefix: "' + data.prefix + '" }';
